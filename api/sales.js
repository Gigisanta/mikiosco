  ...
  try {
    const sale = await dbTransaction(async (client) => {
      ...
      const ticket = await client.query(
        'SELECT COALESCE(MAX(ticket_number), 0) + 1 AS next FROM sales WHERE branch_id = $1 FOR UPDATE',
        [user.branchId],
      )
      const inserted = await client.query(
        'INSERT INTO sales (branch_id, cash_session_id, customer_id, user_id, ticket_number, subtotal, total) VALUES ($1,$2,$3,$4,$5,$6,$6) RETURNING id, ticket_number AS "ticketNumber", total',
        [user.branchId, cashSessionId, customerId, user.id, ticket.rows[0].next, total],
      )
      for (const item of items) {
        const product = await client.query(
          'SELECT id, name, stock, cost_price FROM products WHERE id = $1 AND branch_id = $2 FOR UPDATE',
          [item.productId, user.branchId],
        )
        if (!product.rowCount || Number(product.rows[0].stock) < Number(item.quantity))
          throw new Error(`Stock insuficiente para ${item.name || 'el producto'}.`)
        await client.query(
          'INSERT INTO sale_items (sale_id, product_id, product_name, quantity, unit_price, unit_cost, total) VALUES ($1,$2,$3,$4,$5,$6,$7)',
          [
            inserted.rows[0].id,
            product.rows[0].id,
            product.rows[0].name,
        ...
        await client.query(
          "INSERT INTO stock_movements (branch_id, product_id, type, quantity, reference_id, user_id) VALUES ($1,$2,'SALE',$3,$4,$5)",
          [
            user.branchId,
            product.rows[0].id,
