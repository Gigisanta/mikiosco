  ...
  const [monthly, bestSellers, bestMargins, categories] = await Promise.all([
    dbQuery(
      `SELECT date_trunc('month', s.created_at) AS month, SUM(si.total)::numeric AS revenue,
       ...
       GROUP BY 1 ORDER BY 1`, [user.branchId, months],
    ),
    dbQuery(
      `SELECT si.product_id AS id, si.product_name AS name, SUM(si.quantity)::numeric AS quantity,
       ...
       WHERE s.branch_id=$1 AND s.status='COMPLETED' GROUP BY 1,2 ORDER BY quantity DESC LIMIT 10`, [user.branchId],
    ),
    dbQuery(
      `SELECT si.product_id AS id, si.product_name AS name,
       ...
       WHERE s.branch_id=$1 AND s.status='COMPLETED' GROUP BY 1,2 ORDER BY profit DESC LIMIT 10`, [user.branchId],
    ),
    dbQuery(
      `SELECT COALESCE(c.name,'Sin categoría') AS category, SUM(si.total)::numeric AS revenue
       ...
       WHERE s.branch_id=$1 AND s.status='COMPLETED' GROUP BY 1 ORDER BY revenue DESC`, [user.branchId],
    ),
  ])

  return json(response, 200, { monthly: monthly.rows, bestSellers: bestSellers.rows, bestMargins: bestMargins.rows, categories: categories.rows })
}
